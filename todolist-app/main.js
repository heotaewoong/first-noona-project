...
function render() {
  // 현재 탭(mode)에 맞게 "그때그때" 필터링 (filterList 의존 제거)
  const m = (mode === "inprogress") ? "ongoing" : mode; // 과거 id 호환
  const list =
    m === "all"     ? taskList :
    m === "ongoing" ? taskList.filter(v => !v.isComplete)
                    : taskList.filter(v =>  v.isComplete);
...
function filter(e) {
  // 탭 클릭 시: 모드 변경 + 밑줄/active 갱신 → 바로 렌더
  ...
  render(); // 별도 filterList 계산 없이 그리기
}
